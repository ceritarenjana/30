...
export default function ParticipantsClient({ participants: initialParticipants }: { participants: any[] }) {
  ...
  const { data, error, isLoading, mutate } = useSWR('/api/participants', fetcher, {
    fallbackData: initialParticipants,
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            ...
            <div className="flex space-x-3">
              <select className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-gray-900" value={filterEvent} onChange={e => setFilterEvent(e.target.value)}>
                ...
                {uniqueEvents.map(ev => <option key={String(ev)} value={String(ev)}>{String(ev)}</option>)}
              </select>
            </div>
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Participant</th>
                    <th className="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Event</th>
                    <th className="px-2 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Token</th>
                    ...
                    <th className="px-2 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Registered</th>
                    <th className="px-2 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                  ...
                  {filteredParticipants.map((participant: any) => (
                    <tr key={participant.id} className="hover:bg-blue-50 transition-colors">
                      <td className="px-3 py-2 whitespace-nowrap">
                        <div className="font-semibold text-gray-900 text-sm">{participant.name}</div>
                        {participant.organization && (
                          <div className="text-xs text-gray-500">{participant.organization}</div>
                        )}
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap">
                        <div className="font-medium text-gray-900 text-xs md:text-sm">{participant.event_name}</div>
                        <span className={`inline-block px-2 py-0.5 text-[10px] font-medium rounded-full mt-1 ${participant.event_type === 'Seminar' ? 'bg-blue-100 text-blue-800' : participant.event_type === 'Training' ? 'bg-green-100 text-green-800' : 'bg-purple-100 text-purple-800'}`}>{participant.event_type}</span>
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap font-mono text-[10px] text-gray-700 hidden md:table-cell">{participant.token}</td>
                      <td className="px-3 py-2 whitespace-nowrap">
                        <div className="text-xs text-gray-900 truncate max-w-[120px]">{participant.email}</div>
                        {participant.phone && (
                          <div className="text-[10px] text-gray-500 truncate max-w-[120px]">{participant.phone}</div>
                        )}
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2 py-0.5 text-[10px] font-semibold rounded-full ${participant.is_verified ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{participant.is_verified ? (<><span className="mr-1">✔</span>Verified</>) : (<><span className="mr-1">⏳</span>Pending</>)}</span>
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap text-xs text-gray-500">{formatDateTime(participant.registered_at).split(',')[0]}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in">
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 p-8 flex flex-col items-center animate-fade-in-up">
            ...
            <div className="mb-4 text-center">
              ...
              <div className="text-xs text-gray-400 mb-2">Registered: <span className="font-mono">{selected.registered_at ? formatDateTime(selected.registered_at) : '-'}</span></div>
